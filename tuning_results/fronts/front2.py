...
def plot_pareto_fronts(df, output_path):
    """Plota as fronteiras de Pareto baseadas em F1, F2 e F3."""
    points = df[['F1', 'F2', 'F3']].values
    pareto_fronts, rank = fast_non_dominated_sort(points)

    plt.figure(figsize=(10, 7))
    colors = ['red', 'blue', 'green', 'purple', 'orange', 'brown', 'pink', 'gray', 'olive', 'cyan']

    # Plota cada fronteira com uma cor diferente
    for i, front in enumerate(pareto_fronts):
        front_points = points[front]
        plt.scatter(front_points[:, 0], front_points[:, 1], label=f'Fronteira {i+1}', color=colors[i % len(colors)])
    ...
    # Salvando o gráfico
    filename = f"{output_path}/fronteiras_pareto.png"
...
# Gerar gráfico das fronteiras de Pareto
